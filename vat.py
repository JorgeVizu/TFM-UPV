...
# Función para el método Visual Assessment of cluster Tendency (VAT)
def vat(X):
    D = ssd.cdist(X, X)  # Calcula la matriz de distancias
    J = np.argsort(np.argsort(D, axis=1), axis=1)
    I = np.argsort(J.sum(axis=1))
    D = D[I, :][:, I]
    D = D - D.min()
    D = D / D.max()
...
# Visualizar el estadístico de Hopkins
plt.figure(figsize=(6, 4))
plt.text(0.5, 0.5, f"Hopkins statistic: {hopkins_value:.4f}", 
         horizontalalignment='center', 
         verticalalignment='center', 
         fontsize=15, 
         bbox=dict(facecolor='white', alpha=0.5))

plt.title("Visualización del Estadístico de Hopkins")
